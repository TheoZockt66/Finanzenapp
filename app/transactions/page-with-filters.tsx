...
export default function TransactionsPage() {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>([
    ...
    },
    {
      ...
      budget: 'Lebensmittel',
      betrag: 12.80,
      beschreibung: 'Monatskarte öffentliche Verkehrsmittel',
  ...
  const handleUpdateTransaction = (updatedTransaction: Omit<Transaction, 'id'>) => {
    ...
    const newTransaction = { ...updatedTransaction, id: editingTransaction.id };

    setTransactions(transactions.map(t => 
      t.id === editingTransaction.id ? newTransaction : t
    ));

    // Budget-Synchronisation bei Änderungen
    ...
    <Container size="xl" py="md">
      <Stack gap="lg">
        ...
        <Grid>
          <Grid.Col span={{ base: 12, md: 3 }}>
            <Card padding="lg" withBorder>
              ...
              <Text size="xl" fw={700} c="green">€ {totalEinnahmen.toFixed(2)}</Text>
            </Card>
          </Grid.Col>
          <Grid.Col span={{ base: 12, md: 3 }}>
            <Card padding="lg" withBorder>
              <Text size="sm" c="dimmed" mb={5}>Gesamte Ausgaben</Text>
              <Text size="xl" fw={700} c="red">€ {totalAusgaben.toFixed(2)}</Text>
            </Card>
          </Grid.Col>
          <Grid.Col span={{ base: 12, md: 3 }}>
            <Card padding="lg" withBorder>
        ...
        <TransactionForm
          ...
          budgetOptions={budgets.map(b => ({ value: b.name, label: b.name }))}
        />
      </Stack>
    </Container>
  );
